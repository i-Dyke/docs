...
3. Each sensor pixel contains a photodiode that converts photons into electrical charges.

4. The accumulated charge is proportional to the light intensity.

5. An ADC (Analog-to-Digital Converter) converts the analog signal to digital values.

6. These digital values represent the brightness and color information for each pixel.

7. For color images, a Bayer filter mosaic is used, with alternating red, green, and blue filters over the pixels.

8. Demosaicing algorithms interpolate the full color information for each pixel.
